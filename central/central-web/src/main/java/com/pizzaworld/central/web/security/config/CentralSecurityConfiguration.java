package com.pizzaworld.central.web.security.config;
...
@Configuration
@EnableWebSecurity
public class CentralSecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    public void configAuthentication(AuthenticationManagerBuilder auth) throws Exception {
        auth.jdbcAuthentication().dataSource(dataSource)
                ...
                .usersByUsernameQuery("select username, password , 1 as enabled from central_user where username=?")
                .authoritiesByUsernameQuery("select u.username, r.role from central_role r, central_user u, user_role ur where u.id= ur.user_id and r.id = ur.role_id and u.username = ? ");
    }
